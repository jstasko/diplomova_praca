...
def output_vtk_cylinder_open(cyl_shape, n, out_file, closed=False):
    """
    Outputs the VTK files for visualisation of a cylinder in e.g. Paraview.
    ...
    axis = cyl_shape.axis/np.linalg.norm(cyl_shape.axis)
    length = cyl_shape.length  # length is the full height of the cylinder
    radius = cyl_shape.radius
    center = cyl_shape.center
...
def output_vtk_hollow_cone_open(hollow_shape, n, out_file, closed=False):
    """
    Outputs the VTK files for visualisation of a hollow cone in e.g. Paraview.
    ...
    direction : :obj:`int`
     ...
     tg opening angle * radius = length

    """


    axis = hollow_shape.axis/np.linalg.norm(hollow_shape.axis)
    inner_radius = hollow_shape.inner_radius
    outer_radius = hollow_shape.outer_radius
    opening_angle = hollow_shape.opening_angle
    width = hollow_shape.width
    center = hollow_shape.center

    length =(outer_radius - inner_radius) / math.tan(opening_angle)
    ...
    for i in range(0, n):
        output_file.write(
            str(p1[0] + outer_radius * (np.cos(i * alpha) * v1[0] + np.sin(i * alpha) * v2[0])) + " " +
            str(p1[1] + outer_radius * (np.cos(i * alpha) * v1[1] + np.sin(i * alpha) * v2[1])) + " " +
            str(p1[2] + outer_radius * (np.cos(i * alpha) * v1[2] + np.sin(i * alpha) * v2[2])) + "\n")
    ...
    for i in range(0, n):
        output_file.write(
            str(p1[0] + inner_radius * (np.cos(i * alpha) * v1[0] + np.sin(i * alpha) * v2[0])) + " " +
            str(p1[1] + inner_radius * (np.cos(i * alpha) * v1[1] + np.sin(i * alpha) * v2[1])) + " " +
            str(p1[2] + inner_radius * (np.cos(i * alpha) * v1[2] + np.sin(i * alpha) * v2[2])) + "\n")
