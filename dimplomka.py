...
def get_poistions(x_position, y_position, z_position, r_tunnel, r_cell, count, offset):
    """
    Returns full position for specified X and Z using radius of tunnel and cell
    :param x_position: X position
    :param z_position: Z position
    :param r_tunnel: Radius of the tunnel
    :param r_cell: Radius of the cell
    :return: Full position with random Y position
    """

    if z_position < r_cell or z_position > 2 * r_tunnel - r_cell:
        raise Exception('Wrong parameters! Cannot generate position.')
    if r_tunnel < r_cell:
        raise Exception('Wrong parameters! Cannot generate position.')

    generate_max = math.sqrt(math.pow(r_tunnel - r_cell, 2) -
                             math.pow(r_tunnel - z_position if z_position < r_tunnel else z_position - r_tunnel, 2))

    # random Y position
    positions = []
    for i in range(count):
        x = x_position + offset * i

        y = random.uniform(r_tunnel - generate_max, r_tunnel + generate_max)

        positions.append([x, y, z_position])
    return positions

def check_position_in_stenosis(x, modulo, x_position_start, x_position_end):
    return x % modulo > x_position_end and x % modulo < x_position_start

def get_platelet_positions(modulo, x_position_end, x_position_start, y_position, z_position, r_tunnel, r_platelet, count, r_cell, cellPositions, offset):
    if z_position < r_cell or z_position > 2 * r_tunnel - r_cell:
        raise Exception('Wrong parameters! Cannot generate position.')
    if r_tunnel < r_cell:
        raise Exception('Wrong parameters! Cannot generate position.')

    generate_max = math.sqrt(math.pow(r_tunnel - r_platelet, 2) -
                             math.pow(r_tunnel - z_position if z_position < r_tunnel else z_position - r_tunnel, 2))

    positions = []
    for i in range(count):
        positionNotValidCell = True
        positionNotValidPlatelet = True
        notAdded = True
        print(str(i))
        print("----")
        while (notAdded):
            x = x_position_start + offset * i
            if (x % modulo > x_position_end and x % modulo < x_position_start):
                y = y_position
                newPosition = [x, y, z_position]
                positions.append(newPosition)
                break
            else:
                y = random.uniform(r_tunnel - generate_max, r_tunnel + generate_max)
                z = random.uniform(r_tunnel - generate_max, r_tunnel + generate_max)

            newPosition = [x, y, z_position]
            for p in range(len(cellPositions)):
                if (((x + 1.5) % modulo) >= cellPositions[p][0] and ((x - 1.5) % modulo) <= cellPositions[p][0]):
                    if (y > cellPositions[p][1] and ((y - r_platelet) > (cellPositions[p][1] + r_cell))):
                        positionNotValidCell = False
                        break
                    elif (y < cellPositions[p][1] and ((y + r_platelet) < (cellPositions[p][1] - r_cell))):
                        positionNotValidCell = False
                        break
                    else:
                        positionNotValidCell = True
            ...
            if (len(positions) == 0 and positionNotValidCell == False):
                positions.append(newPosition)
                notAdded = False
                break;

            if positionNotValidCell == False:
                positions.append(newPosition)
                notAdded = False

    return positions

def get_platelet_position(modulo,start_tunel, end_tunel, x_position_start, y_position, z_position, r_tunnel, r_platelet, r_cell, cellPositions, offset):
    if z_position < r_cell or z_position > 2 * r_tunnel - r_cell:
        raise Exception('Wrong parameters! Cannot generate position.')
    if r_tunnel < r_cell:
        raise Exception('Wrong parameters! Cannot generate position.')
    ...
    i = 0
    while (notAdded):
        # x = random.uniform(x_position_start, x_position_start + modulo)
        x = x_position_start + offset * i
        y = random.uniform(r_tunnel - generate_max, r_tunnel + generate_max)
        newPosition = [x, y, z_position]
        print(newPosition)
        if x % modulo >= start_tunel and x % modulo <= end_tunel:
            i = i + 1
            continue
        helperArr = []
        for p in range(len(cellPositions)):
            if (x + r_cell >= cellPositions[p][0] and (x - r_cell <= cellPositions[p][0])):
                if (y > cellPositions[p][1] and ((y - r_platelet) > (cellPositions[p][1] + r_cell))):
                    helperArr.append(False)
                elif (y < cellPositions[p][1] and ((y + r_platelet) < (cellPositions[p][1] - r_cell))):
                    helperArr.append(False)
                else:
                    helperArr.append(True)
            else :
                helperArr.append(False)

        result = all(element == False for element in helperArr)
        if (result == True):
            return newPosition
        i = i + 1
...
positions = get_poistions(x_position_inner_pore + length_cone + length_pore, y_position_inner_pore ,z_position_inner_pore, outer_pore_r, r_red_cell, red_cells_count, data["rbc_offset"])
# positions = [[x_position_inner_pore, y_position_inner_pore, z_position_inner_pore]]
...
# platelet_positions = get_platelet_positions(outer_length, x_position_inner_pore - length_pore, x_position_inner_pore + length_cone + length_pore, y_position_inner_pore ,z_position_inner_pore, outer_pore_r, r_platelet_cell, platelets_count, r_red_cell, positions, data["plt_offset"])
# platelet_positions = [[x_position_inner_pore - 1.5, y_position_inner_pore + 1.5, z_position_inner_pore]]
...
boundaries = []
# --------------CONE-----------------
tmp_hollow_cone = shapes.HollowCone(center=[(x_position_inner_pore - (length_pore/2)) - length_cone/2, y_position_inner_pore, z_position_inner_pore], axis=[1.0, 0.0, 0.0], width=1, direction=1, opening_angle=opening_angle, inner_radius=inner_radius_hollow, outer_radius=outside_radius_hollow)
...
for i in range(1, maxCycle):
    ...
    if (count_to_add > 0):
        ...
        for iterator in range(count_to_add):
            ...
            # vytvorenie pozicie
            new_position = get_platelet_position(outer_length, start_of_stenosis, end_of_stenosis, x_position_outer_pore + outer_length/2 + r_red_cell, y_position_inner_pore ,z_position_inner_pore, outer_pore_r, r_platelet_cell, r_red_cell, help_positions, data["plt_offset"])
